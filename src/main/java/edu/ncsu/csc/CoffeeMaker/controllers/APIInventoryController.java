...
public class APIInventoryController extends APIController {
	 ...
	 /**
     * Inventory Service object, to be autowired in by Spring to allow for
    ...
    */
    ...
    /**
     * IngredientTypeService object, to be autowired in by Spring to allow for
     * manipulating the IngredientType model
     */
    @Autowired
    ...
    public ResponseEntity updateInventory ( @RequestBody final Inventory inventory ) {
        ...
        inventoryCurrent.updateIngredients( inventory );
    ...
    public ResponseEntity addInventory ( @RequestBody final Ingredient ingredient) {
        ...
        if(inventoryCurrent.getIngredient(type) != null) {
        	return new ResponseEntity( "Ingredient is already in Inventory", HttpStatus.CONFLICT);
        }
        //if this is the first time we've seen this ingredient type
        if(typeService.findByName(type.getName()) != null) {
        	ingredient.setIngredientType(typeService.findByName(type.getName()));
        }
        typeService.save(type); 
        ingredientService.save(ingredient);
        inventoryCurrent.add(ingredient);
        service.save( inventoryCurrent );
        return new ResponseEntity( inventoryCurrent, HttpStatus.OK );
    }
    
    /**
     * REST API endpoint to provide delete access to CoffeeMaker's singleton
     * Inventory. This will update the Inventory of the CoffeeMaker by deleting 
     * the ingredients from the Inventory 
     *
     * @param ingredient
     *            amounts to add to inventory
     * @return response to the request
    ...
    public ResponseEntity deleteIngredient(@RequestBody final Ingredient ingredient) {
    	final Inventory inventoryCurrent = service.getInventory();
        IngredientType type = ingredient.getIngredientType();
        if(inventoryCurrent.getIngredient(type) == null) {
        	return new ResponseEntity( "Ingredient is not in Inventory", HttpStatus.CONFLICT);
        	}
        inventoryCurrent.deleteIngredient(ingredient);
        service.save( inventoryCurrent );
        return new ResponseEntity( inventoryCurrent, HttpStatus.OK );
    }
