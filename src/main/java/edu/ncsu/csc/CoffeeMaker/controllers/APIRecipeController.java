...
public class APIRecipeController extends APIController {
    ...
    */
    ...
    public ResponseEntity createRecipe ( @RequestBody final Recipe recipe ) {
        ...
        if ( service.count() < 3 ) {
        	for (Ingredient i : recipe.getIngredients()) {  
        		i.setIngredientType(ingredientTypeService.findByName(i.getIngredientType().getName()));
     ...
     * 
     * @param recipe
     *            recipe to be collect updates from
     * @return ResponseEntity indicating success if the Recipe could be updated to
     *         the inventory, or an error if it could not be
     */
    @PutMapping ( BASE_PATH + "/recipes/" )
    public ResponseEntity updateRecipe ( @RequestBody final Recipe recipe ) {
    	Recipe old = service.findByName( recipe.getName() );
        if ( null == old ) {
            return new ResponseEntity( errorResponse( "Recipe with the name " + recipe.getName() + " does not already exist!" ),
                    HttpStatus.CONFLICT );
        }
        else {
        	// update -> remove old version, save new version
        	service.delete(old);
        	service.save(recipe);
